...
public class Model {
	...
	public void draw(Canvas c, float interpol) {
		...
		// p.setColor((duration >= (DURATION_LIMIT / 2)) ? 0xffff0000 :
		// 0xff0000ff);
...
//		drawValuesText(c, interpol);
	...
	private void drawValuesColor(Canvas c, float interpol) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (drawVals[i] < 0) {
				r.bottom += (int) ((0 - drawVals[i]) * scale);
			} else {
				r.top -= (int) (drawVals[i] * scale);
			}
	...
	private float[] getDrawValues(float interpol) {
		...
		curVals[0] = (nX-x)*interpol + x;
		curVals[1] = (nY-y)*interpol + y;
		curVals[2] = (nZ-z)*interpol + z;
...
//		nextVals[2] = (nZ - z) * nextFraction + z;
	...
	private void drawValuesText(Canvas c, float interpol) {
		...
		for (int i = 0; i < 3; i++) {
			...
			c.drawText(text, (displayD.width - bounds.width()) / 2, heightInc
					* (i + 1), p);
		}
	...
	private void drawStateText(Canvas c, float interpol) {
		Rect bounds = new Rect();
		Paint p = new Paint();
		String text = (state == DisplayState.STOPPED) ? "Stopped" : "Paused";
		p.setColor(0xff000000);
		p.setAntiAlias(true);
		p.setTextSize(50F);
		p.getTextBounds(text, 0, text.length(), bounds);
